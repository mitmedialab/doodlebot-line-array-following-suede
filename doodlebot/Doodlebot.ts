...
const trimNewtworkStatusMessage = (message: string, prefix: NetworkStatus) =>
...
export default class Doodlebot {
  ...
  constructor() {
    this.ble.promise.then((ble) =>
      ble.onReceive(this.receiveTextBLE.bind(this))
    );
    this.ble.promise.then((ble) =>
      ble.onDisconnect(this.handleBleDisconnect.bind(this))
    );
    this.bleDevice.promise.then(async ({ device, service }) => {
      this.ble.resolve({
        onReceive: (callback) =>
          service.addEventListener("receiveText", callback),
    ...
    this.pc.ontrack = (event) => {
      ...
      const handleVideoFrame = (now) => {
        ...
        ctx.drawImage(
          ...
          0,
          0,
          this.canvasWebrtc.width,
          this.canvasWebrtc.height
        );
        this.webrtcVideo.requestVideoFrameCallback(handleVideoFrame);
      };
      this.webrtcVideo.requestVideoFrameCallback(handleVideoFrame);
  ...
  private parseCommand(text: string) {
    ...
    return lines.map((line) => {
      const [command, ...parameters] = line.split(",").map((s) => s.trim()) as [
        ReceivedCommand,
        ...string[]
      ];
  ...
  private updateNetworkStatus(ipComponent: string, hostnameComponent: string) {
    ...
    const hostname = trimNewtworkStatusMessage(
      hostnameComponent,
      networkStatus.hostnamePrefix
    );
    this.reloadRequired?.(
      "Network settings changed, please reload the page and reconnect to doodlebot."
    );
  }
    ...
    for (const { command, parameters } of this.parseCommand(detail)) {
      ...
      switch (command) {
        case motorCommandReceived:
          ...
          break;
        case sensor.bumper: {
          const [front, back] = parameters.map((parameter) =>
            Number.parseFloat(parameter)
          );
        ...
        case sensor.accelerometer: {
          const [x, y, z] = parameters.map((parameter) =>
            Number.parseFloat(parameter)
          );
        ...
        case sensor.line: {
          const [l1, l2, l3, l4] = parameters.map((parameter) =>
            Number.parseFloat(parameter)
          );
        ...
        case sensor.light: {
          const [red, green, blue, alpha] = parameters.map((parameter) =>
            Number.parseFloat(parameter)
          );
  ...
  private handleBleDisconnect() {
    this.reloadRequired?.(
      "Doodlebot bluetooth disconnected, please reload the page and reconnect to doodlebot."
    );
    for (const callback of this.disconnectCallbacks) callback();
    for (const { target, event, listener } of this.subscriptions)
   ...
   *
   */
  private disableTimers: Partial<
    Record<SensorKey, ReturnType<typeof setTimeout>>
  > = {};

  async disableSensor<T extends SensorKey>(type: T) {
    console.log("DISABLING");
    console.log(this.sensorState[type]);
    if (!this.sensorState[type]) return;
  ...
  extractList(text: string) {
    ...
    const itemNames = Array.from(listItems).map((li) => {
  ...
  async getFacePrediction(type: "face" | "object") {
    console.log("CALLING GET FACE PREDICTION", type);
    const now = Date.now();

    // Clear old stop timers whenever we get a new call
    console.log(this.stopTimers[type]);
    ...
    if (!this.lastCallTime[type] || now - this.lastCallTime[type] > 5000) {
      this.lastCallTime[type] = now;
  ...
  private resetStopTimer(type: "face" | "object") {
    if (this.stopTimers[type]) {
      window.clearTimeout(this.stopTimers[type]);
    }
  ...
  getReadingLocation(axis, type, reading) {
    if (type == "face") {
      ...
      if (type == "apple") {
        ...
        const firstOrange = reading.objects.find(
          (obj) => obj.label === "orange"
        );
        if (firstOrange) {
          if (axis == "x") {
  ...
  async motorCommand(
    type: "steps",
    left: MotorStepRequest,
    right: MotorStepRequest
  ): Promise<void>;
  /**
   *
   * @param type
   ...
   * @param type
   */
  async motorCommand(type: "stop"): Promise<void>;
  async motorCommand(type: MotorCommand, ...args: any[]) {
    const {
      pending: { motor: pending },
    } = this;
    switch (type) {
      case "steps": {
        if (pending) await pending;
        const [left, right] = args as MotorStepRequest[];
        return await this.untilFinishedPending(
          "motor",
          new Promise(async (resolve) => {
            this.isStopped = false;
            await this.sendBLECommand(
              command.motor,
              left.steps,
              right.steps,
              left.stepsPerSecond,
              right.stepsPerSecond
            );
  ...
  async connectToWebsocket() {
    ...
    await this.untilFinishedPending(
      "websocket",
  ...
  async followLine() {
    ...
    while (true) {
      try {
        ...
        lineData =
          firstQuadrant.length > 0
            ? lineData.filter((value) => value[1] < 350)
            : [];
        ...
        if (first) {
          // @ts-ignore
          ({
            motorCommands: newMotorCommands,
            bezierPoints: this.bezierPoints,
            line: this.line,
          } = followLine(lineData, lineData, prevLine, [], [], [], add, true));
        } else {
          // @ts-ignore
          ({
            motorCommands: newMotorCommands,
        ...
        // Debugging statement
        this.cumulativeLine =
          this.cumulativeLine + `${JSON.stringify(this.line)},`;
        ...
        if (this.iterationNumber % iterations == 0) {
          newMotorCommands[0].angle = this.limitArcLength(
            newMotorCommands[0].angle,
            newMotorCommands[0].radius,
            2
          );
          ...
          if (this.motorCommands && !(this.motorCommands[0].distance > 0)) {
            if (this.motorCommands) {
              t = calculateArcTime(
                this.motorCommands[0].radius,
                this.motorCommands[0].angle,
                newMotorCommands[0].radius,
                newMotorCommands[0].angle
              );
            } else {
            ...
            if (lineData.length == 0 || !this.deepEqual(lineData, prevLine)) {
              if (command.distance > 0) {
                this.sendWebsocketCommand(
                  "m",
                  Math.round(12335.6 * command.distance),
                  Math.round(12335.6 * command.distance),
                  500,
                  500
                );
  ...
  splitIntoChunks(uint8Array, framesPerChunk) {
    ...
    for (let i = dataStart; i < uint8Array.length; i += chunkSize) {
      const chunk: number[] = uint8Array.slice(i, i + chunkSize);
      chunks.push(chunk);
    }
    return chunks;
  }
  ...
  recordAudio(numSeconds = 1) {
    ...
    return new Promise<{ context: AudioContext; buffer: AudioBuffer }>(
      (resolve) => {
  ...
  async untilFinishedPending<TKey extends keyof Pending>(
    type: TKey,
    promise: Pending[TKey]
  ) {
